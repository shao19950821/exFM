...
def run_criteo(feature_columns, feature_index, data_train, data_test, device='cpu'):
    ...
    logging.info("period1:test LogLoss:{}".format(round(log_loss(data_test[target].values, period1_model_pred_ans), 4)))
    ...
    # mask多余特征和多余特征交互
    alpha = period1_model.linear.alpha.data.cpu().numpy()
    beta = period1_model.fm.beta.data.cpu().numpy()
    linear_mask_idx = []
    pair_mask_idx = []
